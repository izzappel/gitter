(function () {
  ...
  Git.prototype.log = function (options, then) {
    ...
    var command = ["log", "--pretty=format:%H %ai %s%d %aN %ae %P".replace(/\s+/g, splitter)];
  ...
  Git.prototype.myLog = function (then) {
    var handler = Git.trailingFunctionArgument(arguments);
    var command = ["log", "--graph", "--full-history", "--all", "--pretty=format:%h %d %s (%an, %aD)"];
    return this._run(command, function (err, data) {
      handler && handler(err, !err && data);
    });
  };
